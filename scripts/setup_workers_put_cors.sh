#!/bin/bash
# Workers API Gateway PUTメソッドのCORS設定スクリプト
...
echo "=== Workers API Gateway PUTメソッドのCORS設定を開始 ==="
...
aws apigateway put-method-response \
  ...
  --region ${REGION} 2>/dev/null || echo "PUT method response already exists"
...
aws apigateway put-integration-response \
  ...
  --region ${REGION} 2>/dev/null || echo "PUT integration response already exists"
...
DEPLOYMENT_ID=$(aws apigateway create-deployment \
  ...
  --region ${REGION} \
  --stage-name "prod" \
  --description "Workers PUT API CORS設定" \
  --query "id" \
  --output text)
